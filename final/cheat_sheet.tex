...
Given $\bx_q$, take vote among its k nearest neighbors, or take mean of $f$ values of k nearest neighbors if real-values.
...
\section{Neural Net}
\textbf{Forward}:
$\la{\bz}{i} = \la{\ba}{i-1}\la{\bW}{i-1}, \la{\ba}{i} = \la{f}{i}(\la{\bz}{i})$
\ruler
\textbf{Backward MSE}:\\
$
\pfrac{J_{MSE}}{\la{\bW}{i}} = \la{\bdelta}{i+1}\pfrac{\la{\bz}{i+1}}{\la{\bW}{i}}
                             = \tp{(\la{\ba}{i})}\la{\bdelta}{i+1},
$
$
\la{\bdelta}{i+1} = \frac{1}{\nbs}(\la{\ba}{out}-\by)\odot
                    \dfi{out}(\la{\bz}{out})\tp{(\la{\bW}{out-1})}
                    \cdots
                    \tp{(\la{\bW}{i+1})}
                    \odot\dfi{i+1}(\la{\bz}{i+1})
$
\ruler
\textbf{Backward CEE}:\\
$
\pfrac{J_{CEE}}{\la{\bW}{i}} = \la{\bdelta}{i+1}\pfrac{\la{\bz}{i+1}}{\la{\bW}{i}}
                             = \tp{(\la{\ba}{i})}\la{\bdelta}{i+1}
$
$
\la{\bdelta}{i+1} = \frac{1}{\nbs}
                    [(\la{\ba}{out}-\by)\oslash(\bhy\odot(1-\la{\ba}{out}))]
                    \odot
                    \dfi{out}(\la{\bz}{out})\tp{(\la{\bW}{out-1})}
                    \cdots
                    \tp{(\la{\bW}{i+1})}
